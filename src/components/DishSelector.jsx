    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center">
      <div className="bg-white w-full max-w-md rounded-t-3xl max-h-[78vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-4 border-b border-stone-100">
          <h2 className="font-bold text-stone-800">Elige un plato</h2>
          <button onClick={onClose} className="text-stone-400 cursor-pointer transition-colors hover:text-stone-600">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1">
          {dishes.length === 0 ? (
            ...
            filtered.length === 0 ? (
              <p className="text-center text-stone-400 py-10 text-sm">Sin resultados</p>
            ) : (
              filtered.map((dish) => (
